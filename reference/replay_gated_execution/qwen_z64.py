"""Bound singleton Qwen runtime for the canonical Z64 semantic registry.
...
def resolve(model: nn.Module) -> tuple[nn.Module, nn.Module]:
    ...
    if decoder is None or head is None or not isinstance(getattr(decoder, "layers", None), nn.ModuleList):
        raise ValueError("frozen Qwen decoder/output-head layout mismatch")
    return decoder, head
...
def _score_registry(model: nn.Module, tokenizer: Any, context: QwenContext,
...
def _score_registry_independent(model: nn.Module, tokenizer: Any, context: QwenContext,
                                ids: Tensor, action: Tensor | None,
                                hooks: PhysicalWriteHooks) -> Tensor:
...
class BoundQwenZ64Runtime:
    ...
    def __init__(self, *, fp32_model: nn.Module, bf16_model: nn.Module,
        ...
        if any(parameter.requires_grad for model in (fp32_model, bf16_model)
            ...
            raise ValueError("Qwen parameters must be frozen")
    ...
    def capture_telemetry(self) -> RuntimeTelemetry:
        """Capture candidate-free pre-write bridge states once from FP32 Qwen."""
        model = self.models["fp32"]
        decoder, _ = resolve(model)
